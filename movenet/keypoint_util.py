...
def get_angles(mapped_kp_coords, append=False, output_csv=False):
    ...
    angle_series = mapped_kp_coords.apply(calculate_angles, axis=1, angle_dict=ANGLE_DICT)
    ...
    if output_csv:
        angle_df.to_csv(f"{OUTPUT_FOLDER}keypoints_with_angles.csv", index=False)
    return angle_df
...
def calculate_angles(row, angle_dict):
    ...
    for angle_key, keypoints in angle_dict.items():
        keypoint_coords = [tuplize_coordinates(row, [keypoint]) for keypoint in keypoints]
